...
class _CalendarDatePicker2State extends State<CalendarDatePicker2> {
    ...
    setState(() {
      ...
      switch (calendarType) {
        ...
        case CalendarDatePicker2Type.range:
          ...
          } else if (isSelectedDayBeforeStartDate && widget.config.rangeBidirectional != true) {
            selectedDates = [value, null];
          } else {
  ...
  Widget build(BuildContext context) {
    ...
    final rowHeight = widget.config.dayMaxWidth != null ? (widget.config.dayMaxWidth! + 2) : _dayPickerRowHeight;
    ...
    return widget.config.calendarViewMode == CalendarDatePicker2Mode.scroll
        ...
        : Stack(
            children: <Widget>[
              SizedBox(
                height: (widget.config.controlsHeight ?? _subHeaderHeight) + maxContentHeight,
                child: _buildPicker(),
              ),
