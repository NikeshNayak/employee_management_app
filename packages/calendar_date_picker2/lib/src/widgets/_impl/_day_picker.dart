...
class _DayPickerState extends State<_DayPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final Color daySplashColor = widget.config.daySplashColor ?? selectedDayBackground.withValues(alpha: 0.38);
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        if (isDateInBetweenRangePickerSelectedDates) {
          final isStartDate = DateUtils.isSameDay(DateUtils.dateOnly(widget.selectedDates[0]), dayToBuild);
          final isEndDate = DateUtils.isSameDay(DateUtils.dateOnly(widget.selectedDates[1]), dayToBuild);
          var rangePickerIncludedDayDecoration = BoxDecoration(
            color: widget.config.selectedRangeHighlightColor ??
                (widget.config.selectedDayHighlightColor ?? selectedDayBackground).withValues(alpha: 0.15),
          );
          if (widget.config.selectedRangeDecorationPredicate != null) {
            rangePickerIncludedDayDecoration = widget.config.selectedRangeDecorationPredicate?.call(
          ...
          final rangePickerIncludedDayHighlight = widget.config.selectedRangeHighlightBuilder?.call(
            dayToBuild: dayToBuild,
            isStartDate: isStartDate,
            isEndDate: isEndDate,
          );
...
class _DayPickerGridDelegate extends SliverGridDelegate {
  ...
  SliverGridLayout getLayout(SliverConstraints constraints) {
    ...
    if (config?.calendarViewMode == CalendarDatePicker2Mode.scroll && config?.hideScrollViewMonthWeekHeader == true) {
      totalRowsCount -= 1;
    }
    var rowHeight = config?.dayMaxWidth != null ? (config!.dayMaxWidth! + 2) : _dayPickerRowHeight;
