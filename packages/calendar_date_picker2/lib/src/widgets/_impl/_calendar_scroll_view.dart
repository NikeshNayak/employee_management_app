...
class _CalendarScrollViewState extends State<_CalendarScrollView> {
  ...
  void initState() {
    ...
    if (!initialDate.isBefore(widget.config.firstDate) && !initialDate.isAfter(widget.config.lastDate)) {
      _initialMonthIndex = DateUtils.monthDelta(widget.config.firstDate, initialDate);
    }
  ...
  int get _numberOfMonths => DateUtils.monthDelta(widget.config.firstDate, widget.config.lastDate) + 1;
  ...
  Widget _buildMonthItem(BuildContext context, int index, bool beforeInitialMonth) {
    ...
    final DateTime month = DateUtils.addMonthsToMonthDate(widget.config.firstDate, monthIndex);
    ...
    var rowHeight = widget.config.dayMaxWidth != null ? (widget.config.dayMaxWidth! + 2) : _dayPickerRowHeight;
        ...
        widget.config.scrollViewMonthYearBuilder?.call(month) ??
            Row(
              children: [
                ...
                Container(
                  height: rowHeight,
                  margin: const EdgeInsets.symmetric(horizontal: _monthPickerHorizontalPadding),
                  padding: const EdgeInsets.symmetric(horizontal: _monthPickerHorizontalPadding),
                  child: Center(
                    child: Text(
                      widget.config.modePickerTextHandler?.call(monthDate: month) ??
        ...
        if (widget.config.hideScrollViewTopHeader != true)
          Column(
            ...
            children: [
              ...
              if (_showWeekBottomDivider && widget.config.hideScrollViewTopHeaderDivider != true)
...
class _CalendarKeyboardNavigatorState extends State<_CalendarKeyboardNavigator> {
  ...
  DateTime? _nextDateInDirection(DateTime date, TraversalDirection direction) {
    ...
    if (!nextDate.isBefore(widget.config.firstDate) && !nextDate.isAfter(widget.config.lastDate)) {
      return nextDate;
    }
    return null;
  }
...
class _CalendarScrollViewHeader extends StatelessWidget {
  ...
  List<Widget> _dayHeaders(TextStyle? headerStyle, MaterialLocalizations localizations) {
    ...
    final firstDayOfWeek = config.firstDayOfWeek ?? localizations.firstDayOfWeekIndex;
    ...
    for (int i = firstDayOfWeek; true; i = (i + 1) % 7) {
      ...
      result.add(ExcludeSemantics(
        child: Center(
          child: config.weekdayLabelBuilder?.call(weekday: i, isScrollViewTopHeader: true) ??
              Text(
                weekday,
                style: config.scrollViewTopHeaderTextStyle ?? config.weekdayLabelTextStyle ?? headerStyle,
              ),
        ),
      ));
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      constraints:
          BoxConstraints(maxHeight: config.dayMaxWidth != null ? config.dayMaxWidth! + 2 : _dayPickerRowHeight),
      child: LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
        final double tileWidth = (constraints.maxWidth - _monthPickerHorizontalPadding * 2) / DateTime.daysPerWeek;
