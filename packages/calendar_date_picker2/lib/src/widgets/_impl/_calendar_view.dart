...
class _CalendarViewState extends State<_CalendarView> {
  ...
  void initState() {
    ...
    _pageController = widget.config.dayViewController ??
        PageController(initialPage: DateUtils.monthDelta(widget.config.firstDate, _currentMonth));
  ...
  void didUpdateWidget(_CalendarView oldWidget) {
    ...
    if (widget.initialMonth != oldWidget.initialMonth && widget.initialMonth != _currentMonth) {
      ...
      _ambiguate(WidgetsBinding.instance)!.addPostFrameCallback(
        (Duration timeStamp) =>
            _showMonth(widget.initialMonth, jump: widget.config.animateToDisplayedMonthDate != true),
      );
    }
  }
                ...
                if (widget.config.hideLastMonthIcon != true)
                  InkWell(
                    ...
                    child: Padding(
                      ...
                      child: SizedBox(
                        height: widget.config.lastMonthHeight,
                        width: widget.config.lastMonthWidth,
                        child: SvgPicture.asset(
                          'assets/images/arrow_dropdown_left.svg',
                          height: widget.config.lastMonthHeight,
                          width: widget.config.lastMonthWidth,
                          color: _isDisplayingFirstMonth ? const Color(0xFFE5E5E5) : const Color(0xFF949C9E),
                        ),
                      ),
                    ),
                  ),
                ...
                if (widget.config.hideNextMonthIcon != true)
                  InkWell(
                    ...
                    child: Padding(
                      padding: const EdgeInsets.symmetric(vertical: 5, horizontal: 12.0),
                      child: SizedBox(
                        height: widget.config.nextMonthHeight,
                        width: widget.config.nextMonthWidth,
                        child: SvgPicture.asset(
                          'assets/images/arrow_dropdown_right.svg',
                          height: widget.config.nextMonthHeight,
                          width: widget.config.nextMonthWidth,
                          color: _isDisplayingLastMonth ? const Color(0xFFE5E5E5) : const Color(0xFF949C9E),
                        ),
                      ),
                    ),
                  ),
            ...
            child: FocusableActionDetector(
              ...
              child: _FocusedDate(
                ...
                child: PageView.builder(
                  ...
                  scrollDirection: widget.config.dayModeScrollDirection ?? Axis.horizontal,
                ),
              ),
            ),
          ),
  ...
  List<Widget> _buildMonthYear() {
    final ThemeData themeData = Theme.of(context);
    final ColorScheme colorScheme = themeData.colorScheme;
    final TextTheme textTheme = themeData.textTheme;
    final Color controlColor = colorScheme.onSurface.withValues(alpha: 0.60);
    final controlTextStyle = widget.config.controlsTextStyle ?? textTheme.titleSmall?.copyWith(color: controlColor);
    const modePickerMainAxisAlignment = MainAxisAlignment.center;
    ...
    return [
      Row(
        ...
        children: [
          Semantics(
            label: widget.config.semanticsDictionary?[CalendarDatePicker2SemanticsLabel.selectMonth] ??
          ...
          Semantics(
            label: widget.config.semanticsDictionary?[CalendarDatePicker2SemanticsLabel.selectYear] ??
                MaterialLocalizations.of(context).selectYearSemanticsLabel,
            excludeSemantics: true,
            button: true,
            child: SizedBox(
